...
def visualize(templist, height, width):
    ...
    for w in range(9):
        for h in range(6):
            img[(h * 224):(h * 224 + 224), (w * 224):(w * 224 + 224), :] = templist[w * 6 + h]
    for w in range(9):
        img[(height - 224):height, (w * 224):(w * 224 + 224), :] = templist[54 + w]
    for h in range(6):
        img[(h * 224):(h * 224 + 224), (width - 224):width, :] = templist[54 + 9 + h]
    img[(height - 224):height, (width - 224):width, :] = templist[-1]
    return img
...
def generation(baseroot, saveroot, imglist, colornet):
    for i in range(len(imglist)):		#len(imglist)
        ...
        for w in range(9):
            for h in range(6):
                inimg = torchimg[:, :, (h * 224):(h * 224 + 224), (w * 224):(w * 224 + 224)]
                out_rgb = test(inimg, colornet)
                templist.append(out_rgb)
        for w in range(9):
            inimg = torchimg[:, :, (height - 224):height, (w * 224):(w * 224 + 224)]
            out_rgb = test(inimg, colornet)
            templist.append(out_rgb)
        for h in range(6):
            inimg = torchimg[:, :, (h * 224):(h * 224 + 224), (width - 224):width]
            out_rgb = test(inimg, colornet)
            templist.append(out_rgb)
        inimg = torchimg[:, :, (height - 224):height, (width - 224):width]
        out_rgb = test(inimg, colornet)
        templist.append(out_rgb)
        # Visualize
